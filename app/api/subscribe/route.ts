...
export async function POST(req: Request) {
  try {
    ...
    const ip =
      ...
      "unknown-ip";
    const { success: isAllowed, resetTime } = await checkRateLimit(
      `subscribe_${ip}`,
      {
    ...
    if (!apiKey || apiKey === "demo-resend-key") {
      throw new Error(
        "Resend API key is missing or invalid. Check your settings.",
      );
    }
    ...
    for (const promo of allPromos) {
      ...
      if (promo.max_uses !== null && promo.current_uses >= promo.max_uses)
        continue;
      if (promo.start_date && now < new Date(promo.start_date).getTime())
        continue;
    ...
    if (!selectedPromo) {
      return NextResponse.json(
        {
          error:
            "No active promo code is currently available to send. We have added you to our newsletter list!",
        },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      {
        error:
          "An unexpected error occurred while subscribing. Please try again later.",
      },
      { status: 500 },
    );
  }
