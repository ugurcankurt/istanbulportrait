...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Log confirmed booking creation attempt
    if (process.env.NODE_ENV === "development") {
      console.log(
        `Creating confirmed booking from ${ip} for package ${packageId}, customer: ${customerEmail}`,
      );
    }
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        console.log(
          `✅ Confirmed booking created successfully in ${duration}ms:`,
          booking.id,
        );
      }
