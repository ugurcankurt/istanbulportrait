  ...
  try {
    ...
    const csvRows = await Promise.all(
      eligible.map(async (customer) => {
        ...
        return [
          customer.email ? await hashCustomerData(customer.email) : "",
          customer.phone ? await hashPhoneNumber(customer.phone) : "",
          first ? await hashCustomerData(first.toLowerCase().trim()) : "",
          last ? await hashCustomerData(last.toLowerCase().trim()) : "",
          await hashCustomerData("tr"),
