...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!rateLimitResult.success) {
      logError(new Error("Rate limit exceeded"), { ip, endpoint: "admin-blog-list" });
    ...
    const result = await getAllBlogPosts(params as any);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!rateLimitResult.success) {
      logError(new Error("Rate limit exceeded"), { ip, endpoint: "admin-blog-create" });
      return createRateLimitError(rateLimitResult.resetTime);
    }
