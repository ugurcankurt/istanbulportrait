...
const TARGET_LOCALES = ["tr", "ar", "ru", "es", "zh", "de", "fr", "ro"];
...
export async function POST(req: Request) {
  try {
    ...
    const prompt = `
You are a professional localization expert. Your task is to translate the following English web page title and subtitle into the following languages: ${TARGET_LOCALES.join(", ")}.

English Source:
Title: ${title}
Subtitle: ${subtitle}
${content ? `Content Payload: ${JSON.stringify(content, null, 2)}` : ""}
...
Respond ONLY with a valid minified JSON object mapping each locale code to the translated object. Ensure the JSON format matches exactly this structure:
{
  "tr": {
    "title": "...",
    "subtitle": "..."
    ${content ? `,"content": { /* matching the translated content structure */ }` : ""}
  },
  "ar": { ... }
}
