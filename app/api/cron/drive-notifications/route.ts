...
export async function GET(request: Request) {
  try {
    ...
    if (fetchError) {
      console.error(
        "Failed to fetch bookings for drive notifications:",
        fetchError,
      );
    ...
    for (const booking of bookings) {
      try {
        ...
        const groupedFiles = await getGalleryFilesGrouped(
        ...
        if (updateMetadata) {
          ...
          if (updateError) {
            console.error(
              `Failed to update DB after sending emails for booking ${booking.id}:`,
              updateError,
            );
          }
        }
        ...
        console.error(
          `Failed processing drive notifications for booking ${booking.id}:`,
          err,
        );
      }
    }
