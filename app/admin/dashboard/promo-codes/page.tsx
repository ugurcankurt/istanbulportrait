...
export default function PromoCodesPage() {
  ...
  const handleSave = async () => {
    ...
    if (percentageVal <= 0 || percentageVal >= 100)
      return toast.error("Discount must be between 1 and 99 percentage");
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Create completely custom discount codes (e.g. SPRING20) that users
      ...
      {isFormOpen && (
        <div className="p-6 bg-muted/30 border rounded-lg space-y-4 max-w-2xl">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="text-sm font-semibold mb-1 block">
                Max Allowed Uses (Leave empty for unlimited)
              </label>
              <Input
                type="number"
                min="1"
              ...
              <tr key={p.id} className="hover:bg-muted/5 transition-colors">
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    {p.is_active ? (
                      <span className="text-success font-semibold px-2 py-1 bg-success/10 rounded-md text-xs">
                        ACTIVE
                      </span>
                    ) : (
                      <span className="text-muted-foreground font-semibold px-2 py-1 bg-muted rounded-md text-xs">
                        OFF
                      </span>
                    )}
                  </div>
