...
export default function AvailabilityPage() {
  ...
  const [newSurchargeTime, setNewSurchargeTime] = useState<string>(
    TIME_OPTIONS[0],
  );
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const [fetchedSettings, fetchedBlocked, fetchedSurcharges] =
        await Promise.all([
    ...
    try {
      ...
      toast.error(
        e?.message?.includes("unique")
          ? "This time slot already has a surcharge."
          : "Failed to add surcharge.",
      );
    } finally {
      setIsSaving(false);
    }
    ...
    <div className="space-y-8 w-full">
      <div>
        ...
        <p className="text-muted-foreground mb-6 max-w-3xl">
          Manage global working hours and block specific dates or times from
          being booked. Use this tool to lock out holidays or specific periods.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
        ...
        <Card className="lg:col-span-5 xl:col-span-4 shadow-sm border-border">
          <CardHeader className="bg-muted/20 pb-5 border-b">
            ...
            <CardDescription className="pt-1">
              Set the earliest and latest time generally available for booking.
            </CardDescription>
          </CardHeader>
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(val) =>
                    setSettings({ ...settings, start_time: val || "" })
                  }
                >
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(val) =>
                    setSettings({ ...settings, end_time: val || "" })
                  }
                >
                  <SelectTrigger className="font-bold h-11 bg-background">
                    <SelectValue />
        ...
        <Card className="lg:col-span-5 xl:col-span-4 shadow-sm border-border">
          <CardHeader className="bg-amber-500/5 pb-5 border-b">
            ...
            <CardDescription className="pt-1">
              Add percentage markups to busy hours (e.g., Golden Hour). Base
              prices will be dynamically increased.
            </CardDescription>
          </CardHeader>
          <CardContent className="p-6 space-y-6">
            <div className="flex gap-3 items-end">
              <div className="space-y-2 flex-1">
                ...
                <Select
                  value={newSurchargeTime}
                  onValueChange={(val) => setNewSurchargeTime(val || "")}
                >
                  <SelectTrigger className="font-bold h-11 bg-background">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2 flex-1 relative">
                ...
                <Input
                  ...
                  min="1"
                  max="100"
                  className="pr-8 h-11 font-bold"
                  value={newSurchargePercent}
                  onChange={(e) => setNewSurchargePercent(e.target.value)}
                />
            ...
            <Button
              ...
              disabled={isSaving || !newSurchargePercent}
              className="w-full h-11 font-bold shadow-sm bg-amber-500 hover:bg-amber-600 text-white"
            >
                ...
                {surcharges.map((s) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Badge
                        variant="outline"
                        className="font-bold border-amber-200 text-amber-600 bg-amber-50"
                      >
                        {s.time}
                      </Badge>
                      <span className="text-sm font-bold text-emerald-600">
                        +{s.surcharge_percentage}% Price
                      </span>
                    </div>
                    <Button
                      variant="ghost"
        ...
        <Card className="lg:col-span-7 xl:col-span-8 shadow-sm border-border overflow-hidden lg:row-span-2">
          <CardHeader className="bg-muted/20 pb-5 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <CardDescription className="pt-1">
                Dates and times manually disabled for booking.
              </CardDescription>
            </div>
            <Button
              className="gap-2 shadow-sm font-bold shrink-0"
            ...
            <Dialog open={isAddModalOpen} onOpenChange={setIsAddModalOpen}>
              <DialogContent className="max-w-md">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Select a date and time. Leave time as "All Day" to fully
                ...
                <div className="space-y-5 py-4">
                  <div className="grid grid-cols-2 gap-4">
                      ...
                      <Popover>
                        <PopoverTrigger
                          className={cn(
                            ...
                            !newBlockDate && "text-muted-foreground",
                          )}
                        >
                          <CalendarIcon className="mr-2 h-4 w-4" />
                          {newBlockDate ? (
                            format(newBlockDate, "yyyy-MM-dd")
                          ) : (
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        value={newBlockTime}
                        onValueChange={(val) => setNewBlockTime(val || "")}
                      >
                ...
                <DialogFooter>
                  ...
                  <Button
                    ...
                    disabled={isSaving || !newBlockDate}
                  >
                    Save Block
                  </Button>
                </DialogFooter>
                      ...
                      <TableCell colSpan={4} className="h-48 text-center">
                        <div className="flex flex-col items-center justify-center text-muted-foreground space-y-2 my-8">
                          ...
                          <p className="font-medium text-foreground">
                            No blocked slots currently
                          </p>
                          <p className="text-sm">
                            Your schedule is completely open based on your
                            working hours.
                          </p>
                        </div>
                      </TableCell>
                      ...
                      >
                        ...
                        <TableCell className="text-right pr-6">
                          <Button
                            variant="ghost"
                            size="icon"
                            className="opacity-0 group-hover:opacity-100 transition-opacity text-destructive hover:bg-destructive hover:text-white"
