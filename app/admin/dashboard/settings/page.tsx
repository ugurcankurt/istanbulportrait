        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                Google Ads hesabınızın ID'si. Google Ads → Admin → Account
                details sayfasından alınır.
              </p>
            </div>

            <div className="space-y-2">
              <Label>Purchase Conversion Label</Label>
              <Input
                value={settings.google_ads_purchase_label || ""}
                onChange={(e) =>
                  updateSetting("google_ads_purchase_label", e.target.value)
                }
                placeholder="xYz1AbCd2EfGh3Ij"
              />
              <p className="text-xs text-muted-foreground">
                Google Ads → Dönüşümler → &quot;Satın alma&quot; → Etiket kodu.
                Sadece etiket kısmını girin (AW-XX/ olmadan).
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                value={settings.google_ads_lead_label || ""}
                onChange={(e) =>
                  updateSetting("google_ads_lead_label", e.target.value)
                }
                placeholder="aBcD1EfGh2IjKl3"
              />
              <p className="text-xs text-muted-foreground">
                Google Ads → Dönüşümler → &quot;Potansiyel müşteri&quot; →
                Etiket kodu.
              </p>
            </div>

            <div className="space-y-2">
              <Label>Begin Checkout Conversion Label</Label>
              <Input
                value={settings.google_ads_begin_checkout_label || ""}
                onChange={(e) =>
                  updateSetting(
                ...
                placeholder="mNop1QrSt2UvWx3"
              />
              <p className="text-xs text-muted-foreground">
                Google Ads → Dönüşümler → &quot;Ödeme başlatma&quot; → Etiket
                kodu.
              </p>
            </div>

            <div className="space-y-2">
              <Label>Lead Form Webhook Security Key</Label>
              <Input
            ...
            <div className="space-y-2">
              <Label>Developer Token (Opsiyonel)</Label>
              <Input
                type="password"
                value={settings.google_ads_developer_token || ""}
                onChange={(e) =>
                  updateSetting("google_ads_developer_token", e.target.value)
                }
                placeholder="dEvElOpErToKeN..."
              />
              <p className="text-xs text-muted-foreground">
                Google Ads API erişimi için Developer Token. Server-Side
                Conversion API için gereklidir (opsiyonel).
              </p>
            </div>

            <div className="space-y-2">
              <Label>Customer ID (Opsiyonel)</Label>
              <Input
                value={settings.google_ads_customer_id || ""}
                onChange={(e) =>
                  updateSetting("google_ads_customer_id", e.target.value)
                }
                placeholder="123-456-7890"
              />
              <p className="text-xs text-muted-foreground">
                Google Ads müşteri ID&apos;si. Tire olmadan girin (örn:
                1234567890). Server-Side Conversion API için gereklidir
                (opsiyonel).
              </p>
            </div>
          </CardContent>
