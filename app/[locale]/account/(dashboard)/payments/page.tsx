        ...
        <div className="space-y-8">
          {bookings.map((booking: any) => {
            ...
            const remainingBalance = Math.max(
              0,
              Number(booking.total_amount) - totalPaid,
            );

            return (
              <Card
                key={booking.id}
                className="overflow-hidden rounded-2xl border-border/50 shadow-sm hover:shadow-md transition-shadow"
              >
                <div className="bg-gradient-to-br from-primary/5 to-transparent p-6 border-b border-border/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                  <div>
                    ...
                    <p className="text-sm font-medium text-muted-foreground">
                      ...
                      <span className="font-mono text-foreground/80">
                        {booking.id.split("_").pop() || booking.id}
                      </span>
                    </p>
                  </div>
                  ...
                  <div className="w-full md:w-auto flex flex-row items-center gap-6 bg-background p-4 rounded-xl border border-border/50">
                    ...
                    <div className="flex-1 md:flex-none">
                      <p className="text-xs text-emerald-500 uppercase tracking-wider font-semibold mb-1">
                    ...
                    <div className="flex-1 md:flex-none">
                      <p className="text-xs text-amber-500 uppercase tracking-wider font-semibold mb-1">
                        {t("balanceDue")}
                      </p>
                      <p className="text-xl font-bold text-amber-500">
                        {formatCurrency(remainingBalance, "EUR")}
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  {booking.payments && booking.payments.length > 0 ? (
                    <div className="divide-y divide-border/50">
                      {booking.payments.map((payment: any) => (
                        <div
                          key={payment.id}
                          className="p-5 flex justify-between items-center bg-card hover:bg-muted/30 transition-colors"
                        >
                          <div className="flex items-center space-x-4">
                            <div
                              className={`p-3 rounded-xl ${["success", "completed", "paid"].includes(payment.status?.toLowerCase()) ? "bg-emerald-500/10 text-emerald-600" : "bg-muted text-muted-foreground"}`}
                            >
                              {["success", "completed", "paid"].includes(
                    ...
                    <div className="p-8 text-center flex flex-col items-center justify-center">
                      ...
                      <p className="text-sm text-muted-foreground mt-1">
                        Payments are typically processed at the time of booking
                        or in person.
                      </p>
                    </div>
                  )}
