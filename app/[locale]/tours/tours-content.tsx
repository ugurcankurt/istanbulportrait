...
import { getAllIstanbulTours } from "@/lib/viator";
...
export function ToursPageContent({ locale }: ToursPageContentProps) {
  ...
  const [filteredTours, setFilteredTours] = useState<ViatorProduct[]>([]);
  ...
  useEffect(() => {
    async function loadTours() {
      try {
        setLoading(true);
        // Load tours using multi-strategy approach (Basic API limit: ~30 tours max)
        const response = await getAllIstanbulTours(locale);

        if (response.success && response.data) {
          setTours(response.data);
          setFilteredTours(response.data);
        } else {
          setError(response.errorMessage || "Failed to load tours");
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    // Category filter (basic categorization based on title keywords)
    if (categoryFilter !== "all") {
      filtered = filtered.filter((tour) => {
        ...
        switch (categoryFilter) {
          ...
          case "private":
            return title.includes("private") || title.includes("exclusive");
          default:
            return true;
        }
      });
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        case "price_asc":
          return a.pricing.summary.fromPrice - b.pricing.summary.fromPrice;
        case "price_desc":
          return b.pricing.summary.fromPrice - a.pricing.summary.fromPrice;
        default:
          return (
  ...
  }, [tours, searchQuery, sortBy, categoryFilter]);
