...
async function testUpdate() {
  ...
  const { data, error } = await supabase.from("bookings").update({ octo_data: newMeta }).eq("id", "booking_1777705360455.713000").select();
  if (error) console.error("UPDATE ERROR:", error);
  else console.log("UPDATE SUCCESS:", data[0].octo_data);
}
testUpdate();
