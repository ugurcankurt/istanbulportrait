...
class ReviewsService {
  ...
  async fetchGoogleReviews(): Promise<GoogleReview[]> {
    try {
      const response = await fetch(`https://featurable.com/api/v1/widgets/${this.config.widgetId}`, {
        method: 'GET',
        headers: {
...
export const reviewsService = new ReviewsService({
  ...
  widgetId: process.env.FEATURABLE_WIDGET_ID || '94e04b2d-aa06-4057-b95b-f09a8fcf5f38',
  maxReviews: 10,
  sortBy: 'newest'
});
