...
function generateAuthString(request: any, randomString: string, uriPath: string = '/payment/auth'): string {
  try {
...
export const initializePayment = async (paymentRequest: PaymentRequest): Promise<any> => {
  try {
    // Check if we have valid API keys
    if (!API_KEY || API_KEY === 'demo-api-key' || !SECRET_KEY || SECRET_KEY === 'demo-secret-key') {
...
function createDemoPaymentResponse(paymentRequest: PaymentRequest) {
  const isTestCard = paymentRequest.paymentCard.cardNumber === '5528790000000008';
...
export const retrievePayment = async (conversationId: string, token: string): Promise<any> => {
  try {
