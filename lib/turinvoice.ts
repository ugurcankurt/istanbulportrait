...
if (!TURINVOICE_SECRET_KEY) throw new Error("TURINVOICE_SECRET_KEY is not defined");
if (!TURINVOICE_CALLBACK_URL) throw new Error("TURINVOICE_CALLBACK_URL is not defined");
...
export async function turinvoiceGetOrderStatus(
    ...
    try {
        ...
        if (data.paymentUrl) {
            data.paymentUrl = data.paymentUrl.replace("turinvoice.com", "turinvoice.ru");
        }
...
export async function turinvoiceGetQRCode(
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Turinvoice QR code generation failed: ${response.status}`);
        }
