...
export function useIndexNow() {
  ...
  const notifyBookingCreated = useCallback(async (bookingId: string) => {
    const urls = [
  ...
  const notifyPackageUpdated = useCallback(async (packageId?: string) => {
    const urls = [
  ...
  const notifyContentUpdated = useCallback(async (path: string) => {
  ...
  const notifyPageDeleted = useCallback(async (urls: string | string[]) => {
